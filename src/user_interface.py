...
async def _title(attr:str)->...: return ctypes.windll.kernel32.SetConsoleTitleW("Chip Browser")
...
def _center(format:str, string:str):
    ...
    return format[:center+6]+string+format[center+len(string):] # color -> f'{Fore.COLOR+string+Fore.RESET}'
...
def display_single(
    ...
    **kwargs:dict or None
    ...
    if isinstance(_centered, bool) and _centered is False and _attr!='SPACE':
        ...
        print(f'{"":<1}|{_center(" "*(width-offset-2), _attr):<{width-4+len(Fore.CYAN)+(len(Effect.BOLD)+len(Effect.UNDERLINE)+len(Effect.RESET)) if Effect.BOLD and Effect.UNDERLINE and Effect.RESET in _attr else 0}}|')
        ...
        if isinstance(_attr, str):
            ...
            try:
                if len(offsets)==len(_attr):
                    ...
                    for _x in [Effect.UNDERLINE+Effect.BOLD+Fore.CYAN+f'WEBSITE: {_url}{_path} ~ PORT: {_port} ~ PROXY: {_proxy}'+Effect.RESET, 'SPACE']: 
