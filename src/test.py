...
from ChipEngine.color_interpreter import Effect, Fore, Back, Style, _style
from ChipEngine.random_generator import RANDOM
...
class Background(object):
    ...
    def SnowEffect(cls):
        ...
        # prone to being changed later on
        ...
        def move_flake(col, stack, particle, color):
            ...
            # If next row is the end, lets start a new snowflake
            if snowflakes[col][0] + 1 == current_row:
                char = particle
                if not particle:
...
@_LIMITER(arg=1)
def LOAD_DATA() -> str:
    start_width, start_height = terminal_size()
    for _i, _x in enumerate(__ASSETS__):
        exec(_x)
...
def window_geomtry():
    ...
    while True:
        ...
        __width__, __height__ = width, height
